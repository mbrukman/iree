...
Status HostSemaphore::status() const {
  absl::MutexLock lock(&mutex_);
  return status_;
}

StatusOr<uint64_t> HostSemaphore::QueryValue() {
...
Status HostSemaphore::WaitForSemaphores(
  ...
  for (auto& semaphore_value : semaphores) {
    ...
    ASSIGN_OR_RETURN(uint64_t current_value, semaphore->QueryValue());
    if (current_value == UINT64_MAX) {
      // Semaphore has failed. Return the error.
      return semaphore->status();
    } else if (current_value < semaphore_value.value) {

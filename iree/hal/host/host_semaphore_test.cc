...
TEST(HostSemaphoreTest, NoOp) {
  ...
  ASSERT_OK_AND_ASSIGN(uint64_t value, semaphore.QueryValue());
...
TEST(HostSemaphoreTest, NormalSignaling) {
  ...
  EXPECT_EQ(2u, semaphore.QueryValue().ValueOrDie());
  EXPECT_OK(semaphore.Signal(3u));
  EXPECT_EQ(3u, semaphore.QueryValue().ValueOrDie());
  EXPECT_OK(semaphore.Signal(40u));
  EXPECT_EQ(40u, semaphore.QueryValue().ValueOrDie());
}
...
TEST(HostSemaphoreTest, StickyFailure) {
  ...
  EXPECT_EQ(UINT64_MAX, semaphore.QueryValue().ValueOrDie());
